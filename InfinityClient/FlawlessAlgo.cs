...
namespace Flawless2
{
    public class FlawlessAlgo
    {
        ...
        public MemoryStream Encrypt(byte[] contents)
        {
            ...
            output.Write(blocks.ToArray(), 0, (int)blocks.Length);

            output.Position = 0;
            byte[] resultArr = output.ToArray(); resultArr = resultArr.Reverse().Take(resultArr.Length - 2).Reverse().ToArray(); //this extracts the encrypted payload
        ...
        public MemoryStream Decrypt(byte[] contents)
        {
            ...
            ushort textlen = BitConverter.ToUInt16(buffer, 0);
